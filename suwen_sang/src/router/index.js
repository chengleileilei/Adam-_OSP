...
export function adminUser(router, to, next) {
	...
	getMenuAdmin()
		.then(async (res) => {
			...
			let { data } = res
			console.log(data);
			store.dispatch('routesList/setRoutesList', setFilterMenuFun(data, store.state.userInfos.userInfos.roles));
			dynamicRoutes[0].children = data;
			const awaitRoute = await dynamicRouter(dynamicRoutes);
			[...awaitRoute, { path: '*', redirect: '/404' }].forEach((route) => {
				router.addRoute({ ...route });
			});
			setCacheTagsViewRoutes(JSON.parse(JSON.stringify(data)));
			next({ ...to, replace: true });
		})
		.catch((err) => { console.log("添加路由出错：", err); });
}
